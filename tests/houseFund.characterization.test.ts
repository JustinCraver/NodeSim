...
import { computeFixture, jsonRoundTrip, requireNode } from './harness/graphHarness';
...
describe('current house-fund demo', () => {
  it('retains the approved monthly values and target month', () => {
    const result = computeFixture(houseFundFixture);
    ...
    expect(result.customOutputs?.get('savingsAdjuster')?.get('out-1')).toBe(1_350);
  ...
  it('round-trips the checked-in JSON fixture without changing computation', () => {
    ...
    const before = computeFixture(houseFundFixture);
    const after = computeFixture(roundTripped);
